...
public actor NativeSpeechRuntime {
    ...
    private static func sileroModelURL() throws -> URL {
        ...
        if let url = StudyMateResourceBundle.bundle?.url(forResource: filename, withExtension: "bin", subdirectory: "Models")
            ?? StudyMateResourceBundle.bundle?.url(forResource: filename, withExtension: "bin") {
            return url
        }
        let candidates = [
            ...
            URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
                .appendingPathComponent("Sources/StudyMate/Resources/Models/ggml-silero-v6.2.0.bin")
        ].compactMap { $0 }
