...
final class ProjectFileManagerTests: XCTestCase {
    func testCompletedEmptySegmentationStatePersists() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("StudyMate-EmptyProjectTests-\(UUID().uuidString)", isDirectory: true)
    ...
    func testDeleteProjectRemovesMetadataAndWaveformRecords() throws {
        let testDirectory = FileManager.default.temporaryDirectory
            .appendingPathComponent("StudyMate-ProjectDeletionTests-\(UUID().uuidString)", isDirectory: true)
    ...
    func testSaveAndLoadIndividualProjectFile() {
        let testDirectory = FileManager.default.temporaryDirectory
            .appendingPathComponent("StudyMate-ProjectTests-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: testDirectory) }
        let manager = ProjectFileManager(baseDirectory: testDirectory)
    ...
    func testLegacyProjectSchemaIsMigratedAndPreservesEditedSubtitles() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("StudyMate-LegacyProjectTests-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: directory) }
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
    ...
    func testFutureProjectSchemaIsUnavailableAndNotTreatedAsMissing() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("StudyMate-FutureProjectTests-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: directory) }
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
    ...
    func testSaveCreatesRecoverableBackupAndDeleteRemovesIt() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("StudyMate-ProjectBackupTests-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: directory) }
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
    ...
    func testExplicitlyAdoptingChangedMediaPreservesProjectAndCreatesBackup() async throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("StudyMate-ProjectAdoptionTests-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: directory) }
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
    ...
    func testCorruptedCurrentProjectRecoversLatestValidBackup() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("StudyMate-ProjectRecoveryTests-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: directory) }
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
    ...
    func testRapidSavesPersistLatestMetadataSnapshot() throws {
        let testDirectory = FileManager.default.temporaryDirectory
            .appendingPathComponent("StudyMate-CoalescedSaveTests-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: testDirectory) }
        let manager = ProjectFileManager(baseDirectory: testDirectory)
        let mediaURL = testDirectory.appendingPathComponent("lesson.mp3")
        try Data("media".utf8).write(to: mediaURL)
    ...
    func testIdenticalSnapshotDoesNotRewriteProjectMetadata() throws {
        let testDirectory = FileManager.default.temporaryDirectory
            .appendingPathComponent("StudyMate-SnapshotSkipTests-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: testDirectory) }
        let manager = ProjectFileManager(baseDirectory: testDirectory)
        let mediaURL = testDirectory.appendingPathComponent("lesson.mp3")
        try Data("media".utf8).write(to: mediaURL)
    ...
    func testLightweightSaveDoesNotDropPendingWaveformPersistence() throws {
        let testDirectory = FileManager.default.temporaryDirectory
            .appendingPathComponent("StudyMate-WaveformCarryTests-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: testDirectory) }
        let manager = ProjectFileManager(baseDirectory: testDirectory)
        let mediaURL = testDirectory.appendingPathComponent("lesson.mp3")
        try Data("media".utf8).write(to: mediaURL)
    ...
    func testLegacyProjectWithNumericTimestampsAndMissingFieldsLoadsSuccessfully() throws {
        let testDirectory = FileManager.default.temporaryDirectory
            .appendingPathComponent("StudyMate-LegacyJSONTests-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: testDirectory) }
        let manager = ProjectFileManager(baseDirectory: testDirectory)
    ...
    func testIsCompatibleHandlesTimestampDriftWhenFileSizeMatches() throws {
        let testDirectory = FileManager.default.temporaryDirectory
            .appendingPathComponent("StudyMate-CompatibilityTests-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: testDirectory) }
    ...
    func testOpeningAndResavingUnmodifiedHydratedProjectDoesNotCreateBackup() throws {
        let testDirectory = FileManager.default.temporaryDirectory
            .appendingPathComponent("StudyMate-NoBackupOnUnmodified-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: testDirectory) }
        let manager = ProjectFileManager(baseDirectory: testDirectory)
        let mediaURL = testDirectory.appendingPathComponent("lesson.mp3")
        try FileManager.default.createDirectory(at: testDirectory, withIntermediateDirectories: true)
